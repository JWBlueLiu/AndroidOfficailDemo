...
public class PickContact extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        ((Button) findViewById(R.id.pick_contact)).setOnClickListener(
            new ResultDisplayer("Selected contact", ContactsContract.Contacts.CONTENT_ITEM_TYPE));
        ((Button) findViewById(R.id.pick_person)).setOnClickListener(
            new ResultDisplayer("Selected person", "vnd.android.cursor.item/person"));
    ...
    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (data != null) {
            ...
            if (uri != null) {
                Cursor c = null;
                try {
                    c = getContentResolver().query(uri, new String[] { BaseColumns._ID }, null,
                        null, null);
