...
public class TouchPaint extends GraphicsActivity {
    ...
    static final int[] COLORS = new int[] {
        Color.WHITE, Color.RED, Color.YELLOW, Color.GREEN, Color.CYAN, Color.BLUE, Color.MAGENTA,
    };
    ...
    public static class PaintView extends View {
        ...
        private boolean onTouchOrHoverEvent(MotionEvent event, boolean isTouch) {
            ...
            if (action == MotionEvent.ACTION_DOWN
                ...
                for (int i = 0; i < N; i++) {
                    for (int j = 0; j < P; j++) {
                        paint(getPaintModeForTool(event.getToolType(j), mode),
                            ...
                            event.getHistoricalPressure(j, i), event.getHistoricalTouchMajor(j, i),
                ...
                for (int j = 0; j < P; j++) {
                    paint(getPaintModeForTool(event.getToolType(j), mode), event.getX(j),
                        event.getY(j), event.getPressure(j), event.getTouchMajor(j),
                        event.getTouchMinor(j), event.getOrientation(j),
        ...
        private void paint(PaintMode mode, float x, float y, float pressure, float major,
            float minor, float orientation, float distance, float tilt) {
            if (mBitmap != null) {
                if (major <= 0 || minor <= 0) {
         ...
         */
        private void drawSplat(Canvas canvas, float x, float y, float orientation, float distance,
            float tilt, Paint paint) {
